...
import { partiesApi, type PartySummary } from "@/lib/api-client"
  ...
  <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 sm:p-10">
        <div className="space-y-3">
          ...
          <p className="text-sm text-white/70">
            Preview ambience, adjust automation cues, and confirm who&rsquo;s on the co-host roster before doors open.
          </p>
        </div>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        {rooms.map((room) => (
          <Card key={room.name} className="border-white/10 bg-white/[0.02]">
            <CardHeader>
            ...
            <CardContent className="space-y-3 text-sm text-white/75">
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs uppercase tracking-[0.35em] text-white/60">
                {room.status}
              </div>
            </CardContent>
          </Card>
