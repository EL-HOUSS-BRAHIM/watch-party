...
export default function SystemMonitoring() {
    ...
    const fetchSystemData = async () => {
      try {
        ...
        const mockHealth: SystemHealth = {
          cpu: { usage: Math.random() * 40 + 20, cores: 8, temperature: Math.random() * 20 + 50 },
          memory: { used: 12.4, total: 32, percentage: 38.75 },
          disk: { used: 245, total: 500, percentage: 49 },
          network: { inbound: Math.random() * 50 + 20, outbound: Math.random() * 30 + 10 },
          database: { connections: Math.floor(Math.random() * 50) + 20, queries: Math.floor(Math.random() * 1000) + 500, slowQueries: Math.floor(Math.random() * 5) },
          uptime: '15d 8h 23m',
          load: [0.45, 0.68, 0.52],
          timestamp: new Date()
        };
        ...
        const mockLogs: LogEntry[] = [
          ...
          },
          {
            ...
            metadata: { query_time: '2.34s', affected_rows: 1250 }
          },
          {
            id: '3',
            ...
            metadata: { video_id: 'vid_789', error_code: 'PROCESSING_FAILED', retry_count: 2 }
          },
          {
            id: '4',
            ...
            level: 'info',
            service: 'websocket',
            message: 'New party connection established',
            metadata: { party_id: 'party_456', user_count: 5 }
          },
          {
            id: '5',
            ...
            level: 'debug',
            service: 'cache',
            message: 'Cache miss for key: user_sessions_123',
            metadata: { cache_key: 'user_sessions_123', ttl: 3600 }
          }
        ];
  ...
  const filteredLogs = logs.filter(log => {
    ...
    const matchesService = logFilter.service === 'all' || log.service === logFilter.service;
    const matchesSearch = !logFilter.search || 
      log.message.toLowerCase().includes(logFilter.search.toLowerCase()) ||
      log.service.toLowerCase().includes(logFilter.search.toLowerCase());
        ...
        <CardHeader>
          ...
          <div className="flex flex-wrap gap-4 mt-4">
            ...
            <Select 
              ...
              onValueChange={(value) => setLogFilter(prev => ({ ...prev, service: value }))}
            >
