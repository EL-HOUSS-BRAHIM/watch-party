...
setup_environment() {
    ...
    if [ ! -f "$APP_DIR/backend/.env" ]; then
        ...
        print_warning "Backend .env created. Key settings you may want to review:"
        print_warning "- SECRET_KEY and JWT keys (generate new ones for production)"
        print_warning "- Database settings (Docker containers vs external AWS RDS)"
        print_warning "- Redis settings (Docker container vs AWS ElastiCache)"
        print_warning "- AWS settings (uses IAM role, no keys needed)"
